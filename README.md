...
- 合入策略为 `git merge -X theirs`，剩余的 modify/delete 冲突（上游删了、本地还留着）一律跟上游：文件还在就取上游版，上游已删就删掉。因此**自有改动请以新增文件为主**；对上游已有文件的自有修改在冲突时会被上游版本覆盖（方法注册走 `init()`，例如 option enum 的展示项被覆盖不影响功能）。
- 若合并仍失败（极端情况），在 fork 里手动合并上游解决冲突后，删除对应失败 run 再手动触发一次即可。
