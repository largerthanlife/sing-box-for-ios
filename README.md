...
| `upstream_tag` | 上游 tag 或分支做底再叠 overlay。默认 `v1.14.0`（稳定 tipa + apple `Bump version`）；填 `testing` 会配 apple `dev`，曾出现装完秒退 | `v1.14.0` |
...
- `scripts/tipa-version.sh`：从 `tag_name` 抽出合法的 `CFBundleShortVersionString` / 单调 `CFBundleVersion`（避免 `1.14.0-testing` 这类非法 marketing version）
