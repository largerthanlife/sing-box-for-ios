...
3. 编译并在 Releases 发布 `sing-box-<版本号>.tipa`，上游是测试版则标记为 prerelease
...
| `update_apple` | 是否重新拉取最新 sing-box-for-apple | `false` |
...
- `scripts/build-tipa.sh`：环境变量驱动的完整构建（clone → 可选合入上游 tag → gomobile → xcodebuild → ldid → tipa）
