...
- 合入策略为 `git merge -X theirs`：上游 `testing` 会被 rebase/force-push（历史改写会产生大面积冲突），冲突时一律取上游版本。因此**自有改动请以新增文件为主**；对上游已有文件的自有修改在冲突时会被上游版本覆盖（方法注册走 `init()`，例如 option enum 的展示项被覆盖不影响功能）。
